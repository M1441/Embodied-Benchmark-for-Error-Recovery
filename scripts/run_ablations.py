...
ALL_MODES = ["full", "after-only", "swap-test", "text-only"]
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run multi-mode ablations with optional reasoning strategies"
    )
    ...
    parser.add_argument(
        ...
        help="Comma-separated list of modes to run (default: all four)"
    )
    parser.add_argument(
    ...
    parser.add_argument("--estimate-samples", type=int, default=300)
...
def _pick_pricing(model: str, in_override, out_override):
...
def _usd_cost(prompt_tokens, completion_tokens, input_per_1m, output_per_1m):
    return (prompt_tokens / 1e6) * input_per_1m + (completion_tokens / 1e6) * output_per_1m
...
def main():
    ...
    for m in modes:
        if m not in ALL_MODES:
            raise ValueError(f"Unknown mode: {m}. Choose from: {ALL_MODES}")

    samples = load_jsonl(str(data_path))
    if args.start >= len(samples):
        raise ValueError(f"--start {args.start} out of range for {len(samples)} samples")

    limit = 2 if args.dry_run else args.limit
    if args.dry_run:
        print(f"[dry-run] Running 2 samples per mode for validation")
    ...
    for mode in modes:
        ...
        if args.dry_run:
            dry_ids = {row.get("sample_id") for row in rows if row.get("sample_id")}
            eval_samples = [s for s in samples if s.get("sample_id") in dry_ids]
        else:
            if limit > 0:
        ...
        report["ci95"] = {
            "action_accuracy": bootstrap_mean_ci95(
                details["action_correct_flags"], n_boot=args.bootstrap_samples
            ),
            "macro_f1": bootstrap_macro_f1_ci95(
                details["y_true"], details["y_pred"], n_boot=args.bootstrap_samples
            ),
            ...
            "recovery_rate_at_k": bootstrap_mean_ci95(
                details["recovered_flags"], n_boot=args.bootstrap_samples
            ),
    ...
    # Cost projection
    pricing_in, pricing_out = _pick_pricing(
        args.model, args.input_price_per_1m, args.output_price_per_1m
    )
    cost_projection = {}
    total_projected_cost = 0.0
    for mode, stat in mode_token_stats.items():
        n = stat["num_with_usage"]
        avg_p = stat["prompt_tokens_total"] / n if n > 0 else 0.0
        avg_c = stat["completion_tokens_total"] / n if n > 0 else 0.0
        projected = _usd_cost(
            avg_p * args.estimate_samples,
            avg_c * args.estimate_samples,
            pricing_in, pricing_out,
        )
        total_projected_cost += projected
        cost_projection[mode] = {
            ...
            "projected_mode_cost_usd": projected,
        }
    ...
    summary = {
        ...
        "pricing_usd_per_1m_tokens": {"input": pricing_in, "output": pricing_out},
        "cost_projection": {
    ...
    tag = "dry_run_summary" if args.dry_run else "ablation_summary"
    # Include strategy in filename to avoid overwriting vanilla results
    if args.prompt_strategy != "vanilla":
        tag = f"{tag}_{args.prompt_strategy}"
